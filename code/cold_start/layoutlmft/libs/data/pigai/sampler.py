...
class SeqSampler:
    def __init__(self, dataset_meta_path, world_size, rank_id, batch_size, seq_length, epoch=0):
        ...
        self.random_state = np.random.RandomState(20250114)
        self.epoch = epoch
        self._init_dataset()
    ...
    def _init_dataset(self):
        ...
        length_file_path = meta_info['length_file']
        assert os.path.exists(length_file_path)
        self.index_2_length = meta_info['lengths']
        self.total_sentence_len = meta_info['size']
        assert len(self.index_2_length) == self.total_sentence_len
        self._sentence_cache_num = 1e6
...
if __name__ == "__main__":
    sampler = SeqSampler(
        '/train34/mmu/permanent/cxqin/zrzhang6/ChartQA/pfhu6/dataset/pigai/v1/dataset/valid/prompt.mdb.json', 
        1, 0, 2, 4096, epoch=0
    )
    output = []
    for idx in sampler:

...
class Dataset:
    def __init__(self, dataset_meta_path, dataset_pixel_meta_path, seq_length, tokenizer, prm_loss_weight, prm_select_random_num, prm_select_ret_num, prm_fixed_position, orm_loss_weight, orm_token_num):
    ...
    def _get_data(self, sample_id):
        ...
        pixel_values = torch.load(os.path.join('/train34/cog8/permanent/bhwei2/pfhu6/shliu19/datasets/mcts_dataset/pixel_values_2_5', data['pixel_name_string'])).numpy()
        ...
        if self.orm_token_num > 0:
            last_several_index = list(range(len(loss_mask) - self.orm_token_num, len(loss_mask)))
            loss_mask[last_several_index] = self.orm_loss_weight
        score = np.full_like(input_ids, data['score'], dtype=np.float32)
        attention_mask = np.ones_like(input_ids, dtype=np.int32)
        
        return {
            'input_ids':torch.from_numpy(input_ids), 'labels':torch.from_numpy(score),
...
class DataCollator:
    ...
    def __call__(self, batch_data):
        ...
        labels = merge1d([data['labels'] for data in batch_data], -100.0)

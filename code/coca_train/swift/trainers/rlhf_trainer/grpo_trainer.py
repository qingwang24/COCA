...
class GRPOTrainer(RolloutTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _generate_and_score_completions(self, inputs: DataType) -> DataType:
        assert len(inputs) % self.num_generations == 0
        num_query = len(inputs) // self.num_generations
        device = self.accelerator.device
        batch_encoded_inputs = []
        for query_id in range(num_query):
            item = inputs[query_id * self.num_generations]

            data_id = [item['id'] + '.pkl' for item in inputs]
            # 读取保存的pkl
            dataset_path = list(self.train_dataset.download_checksums.keys())[0] # sample/dataset/data/xx.parquet
            parent_dataset_path = str(Path(dataset_path).parent.parent.parent) # sample
            pkl_dataset_path = os.path.join(parent_dataset_path, 'pkl_new') # pkl_list
            # pkl_list = [f for f in os.listdir(pkl_dataset_path) if f.endswith('.pkl')]
            pkl_data_path_list = [os.path.join(pkl_dataset_path, item) for item in data_id]
            # 加载pkl
            outputs = []
            pkl_data_path = pkl_data_path_list[0]
            # for path in pkl_data_path_list:
            with open(pkl_data_path, 'rb') as f:
                data = pickle.load(f)
            
            data_item = data[0]
            data_output = data[1]
            num_generation = data_output['input_ids'].shape[0]
            pixel_values = torch.load(os.path.join('/dmx-csy-mix01/cog3/permanent/qkchang/shliu19/critique/dataset/datasets/mcts_dataset/pixel_values_train', os.path.basename(data_item['pixel_values_path'])))
            
            for gen_id in range(num_generation):
                cur_data_output = {}
                cur_data_output['input_ids'] = data_output['input_ids'][gen_id:gen_id+1].to(device)
                cur_data_output['attention_mask'] = data_output['attention_mask'][gen_id:gen_id+1].to(device)
                cur_data_output['image_grid_thw'] = data_output['image_grid_thw'][gen_id:gen_id+1].to(device)
                cur_data_output['completion_mask'] = data_output['completion_mask'][gen_id:gen_id+1].to(device)
                cur_data_output['old_per_token_logps'] = data_output['old_per_token_logps'][gen_id:gen_id+1].to(device)
                cur_data_output['ref_per_token_logps'] = data_output['ref_per_token_logps'][gen_id:gen_id+1].to(device)
                cur_data_output['advantages'] = data_output['advantages'][gen_id:gen_id+1].to(device)
                cur_data_output['pixel_values'] = pixel_values.to(device)
                cur_data_output['logits_to_keep'] = data_output['logits_to_keep']
                batch_encoded_inputs.append(cur_data_output)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        per_token_logps = self._get_per_token_logps(model, inputs) # pi_theta
        
        # Compute the KL divergence between the model and the reference model
        if self.beta != 0.0:
            ref_per_token_logps = inputs['ref_per_token_logps'] # pi_ref, 从ref_model中计算得到的
            per_token_kl = (
                torch.exp(ref_per_token_logps - per_token_logps) - (ref_per_token_logps - per_token_logps) - 1) # GRPO论文中的(4)
        self.epsilon = 0.2
        advantages = inputs['advantages']
        # old_per_token_logps = inputs['old_per_token_logps'] if self.old_policy else per_token_logps.detach() # pi_theta_old, 第一步更新的时候old_policy = model, 所以直接使用per_token_logps
        old_per_token_logps = inputs['old_per_token_logps'] # off_policy时候, 第一轮迭代的时候其实是直接使用ref_per_token_logps的
        coef_1 = torch.exp(per_token_logps - old_per_token_logps) # GRPO loss中的第一项系数, 第一次更新的时候old_policy=policy
        coef_2 = torch.clamp(coef_1, 1 - self.epsilon, 1 + self.epsilon) # PPO中的clip, loss的第二项系数
        per_token_loss1 = coef_1 * advantages.unsqueeze(1) # grpo loss的第一项
        per_token_loss2 = coef_2 * advantages.unsqueeze(1) # grpo loss的第二项
        per_token_loss = -torch.min(per_token_loss1, per_token_loss2) # 取min

        loss = (per_token_loss * completion_mask).sum() / completion_mask.sum() # TODO: 后续需要改成token-level grpo loss, 目前时
        ...
        self.grpo_select_alpha = 0.1
        if self.grpo_select_alpha != 0.0:
            advantages = advantages.unsqueeze(1)
            select_mask = advantages > 0.0000001
            select_mask = select_mask * (completion_mask > 0.)
            select_loss = -1 * torch.sum(select_mask * per_token_logps) / select_mask.sum().clamp(min=1)
            loss = loss + self.grpo_select_alpha * select_loss
